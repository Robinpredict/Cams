    ...
    if config.mode == 'train':
        ...
        if config.task=='Reconstruct':
            if len(config.n_group)>1:
                solver.test_dual_reconstruct2_(method='mean',epoch=-1)#config.best_epoch)
            # solver.test_dual_reconstruct_(method='mean')
            else:
                solver.test_dual_tune_(method='mean',epoch=-1)#config.best_epoch)
...
if __name__ == '__main__':
    ...
    # 256 [24, 12, 8]
    # for d_model in [256]:
        # for n_group in [[24,12,8]]:
    for d_model in [256,512,128]:
        for n_group in [[24,12,8],[8],[8,8],[8,8],[8,8,8],[12,8,4],[8,4]]:
            ...
            model_save_path='TrainedModels/thre'+dataset+'DualReconstruct0'+Groups+R_arch+'dmodel'+str(d_model)+'/'
