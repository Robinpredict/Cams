...
def main(config):
    ...
    if config.mode == 'train':
        best_epoch=solver.train_dual()
        return solver,best_epoch
    elif config.mode == 'test':
        if config.task=='Reconstruct':
            if len(config.n_group)>1:
                solver.test_dual_reconstruct2_(method='mean',epoch=config.best_epoch)
            # solver.test_dual_reconstruct_(method='mean')
            else:
                solver.test_dual_tune_(method='mean',epoch=config.best_epoch)
...
if __name__ == '__main__':
    ...
    save_location='D:/OneDrive - Nanyang Technological University/AD_Data_Normalized'
